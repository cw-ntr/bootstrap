...
/***********************************************
  AT THE MOMENT ADDITIVE !!! (TESTING IF WORKS ALSO, SINCE LESS ERROR PRUNE)
...
resource "google_organization_iam_member" "billing_creator" {
    org_id = var.org_id
...
resource "google_organization_iam_member" "project_creator" {
  ...
  org_id  = local.parent_id
  role    = "roles/resourcemanager.projectCreator"
  member = local.org_project_creators
}
